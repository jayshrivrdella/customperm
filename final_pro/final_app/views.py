...
class PermissionGenerator(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer_class = PermissionGeneratorCustomSerializer(data=request.data)
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if user:
                ...
                data = User.objects.create(email=request.data['email'], password=password, username=request.data['username'])
                serializer_class = CustomSerializer(data=request.data)
                if serializer_class.is_valid():
                    if serializer_class.data['role'] == 'ADMIN':
                        userid = data.userid
                        role_id = Roles.objects.get(role=serializer_class.data['role']).role_id
                        user_role=Userrole.objects.create(u_id=userid,r_id= role_id)
                    elif serializer_class.data['role'] == 'TL':
                        userid = data.userid
                        role_id = Roles.objects.get(role=serializer_class.data['role']).role_id
                        user_role=Userrole.objects.create(u_id=userid, r_id=role_id)
...
class Login(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if password_check:
                serializer = LoginSerializer(mail, many=True)
                token = generate_custom_model_token(mail.userid)
